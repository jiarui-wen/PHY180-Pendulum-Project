...
def exp_cos(t, a, b, T, C):
    return a*bb.np.exp(-t/b) * bb.np.cos(2*math.pi*t/T + C*t**2)
...
filename = 'AAAtAmp3revised.txt'
x, y, xerr, yerr = bb.load_data(filename)

# bb.plot_fit(expon, x, y, xerr, yerr)
bb.plot_fit(exp_cos, x, y, xerr, yerr, xlabel="Time Elapsed(s)", ylabel="Amplitude(rad)", title="")
